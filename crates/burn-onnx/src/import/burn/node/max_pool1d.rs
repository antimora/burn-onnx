...
/// only at run time) is applied beforehand with -inf, which never wins the max, and
/// the indices are shifted back from the padded row to the input's.
fn forward_with_indices(
    ...
    // Reads `length` and the pads bound as `left`, `right`.
