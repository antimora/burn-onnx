...
impl NodeCodegen for onnx_ir::col2im::Col2ImNode {
    ...
    fn forward(&self, scope: &mut ScopeAtPosition<'_>) -> TokenStream {
        ...
        let image = lift(&config.image_shape, 1);
        let kernel = lift(&config.block_shape, 1);
        ...
        let (fold_size, fold_padding) = if symmetric {
            (image, pads_begin)
        } else {
            (
                [
                    image[0] + pads_begin[0] + pads_end[0],
                    image[1] + pads_begin[1] + pads_end[1],
                ],
                [0, 0],
            )
        };
