...
fn main() {
    let artifacts = model_checks_common::artifacts_dir_build("depth-anything-v2");
    let onnx_path = artifacts.join("depth-anything-v2_opset16.onnx");
    ...
    if !onnx_path.exists() {
        ...
        eprintln!();
        eprintln!(
            "This will download the Depth-Anything-v2-Small model and convert it to ONNX format."
        );
        std::process::exit(1);
    }
