...
fn main() {
    let artifacts = model_checks_common::artifacts_dir_build("mediapipe-face-detector");
    let onnx_path = artifacts.join("face_detector.onnx");
    let test_data_path = artifacts.join("face_detector_test_data.pt");
    ...
    if !onnx_path.exists() {
        ...
        eprintln!();
        eprintln!("This will download the MediaPipe face detector and convert it to ONNX.");
        std::process::exit(1);
    }
