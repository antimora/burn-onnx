...
def download_and_convert_model(model_name, output_path):
    """Download SmolLM model from HuggingFace and export to ONNX format."""
    import torch
...
def main():
    parser = argparse.ArgumentParser(description='SmolLM Model Preparation Tool')
    parser.add_argument('--model', type=str, default='smollm-135m',
                        choices=list(SUPPORTED_MODELS.keys()),
                        help=f'SmolLM model to download and prepare (default: smollm-135m)')
    ...
    artifacts_dir = get_artifacts_dir("smollm")
