...
# combination, against a tensor and against a scalar divisor, plus a pair that
# broadcasts ([2, 1] against [3]).
...
def main():
    ...
    graph = helper.make_graph(nodes, "mod_int_fmod", [x, y, d, a, b], [z, zs, zb])
    ...
    z_out, zs_out, zb_out = ReferenceEvaluator(model).run(
