...
def out_len(size, begin, end, kernel, stride, ceil):
    span = size + begin + end - kernel
    n = (-(-span // stride) if ceil else span // stride) + 1
    return n - 1 if ceil and (n - 1) * stride >= size + begin else n


def pool(x, kernel, stride, begin, end, ceil=False):
    n, c, length = x.shape
    out = out_len(length, begin, end, kernel, stride, ceil)
    values = np.zeros((n, c, out), np.float32)
...
def main():
    f = TensorProto.FLOAT
    configs = {
        "sym": dict(kernel_shape=[2], strides=[2], pads=[1, 1], storage_order=1),
        "asym": dict(kernel_shape=[3], strides=[1], pads=[0, 2]),
        "ceil": dict(kernel_shape=[2], strides=[2], pads=[1, 1], ceil_mode=1),
    }
    nodes = [
    ...
    shapes = {"sym": [2, 2, 4], "asym": [2, 2, 7], "ceil": [2, 2, 4], "same": ["N", 2, "L"]}
    outputs = []
    ...
    reference = ort.InferenceSession("maxpool1d_indices.onnx").run(None, {"x": x, "xd": x})
    expected = {
        "sym": pool(x, 2, 2, 1, 1),
        "asym": pool(x, 3, 1, 0, 2),
